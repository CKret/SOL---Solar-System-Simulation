...
- During the intro, body metadata warms incrementally: the initial load starts with authoritative bodies plus objects up to `H <= 12`, then background requests expand one H band at a time through `H <= 25` via `/api/bodies/batch` so the searchable/renderable catalog grows without re-fetching already cached bodies.
