...
- Each comet renders an animated dust tail (3 000 streaming particles, Gaussian cross-section, golden-white gradient fading toward the tip, curvature that strengthens near perihelion to follow orbital lag) and a narrower blue-white ion tail (1 200 particles, straight anti-solar). Both tails use additive blending and are driven by simulated time: they freeze when paused and flow proportionally to the selected simulation speed.
...
- Comet orbit lines are sampled by eccentric anomaly over [0, 2π] anchored to perihelion rather than from a window relative to the current simulation time. This avoids the apparent rotation artefact that arises with time-uniform sampling on highly eccentric orbits, and keeps the orbit line stable regardless of how fast the simulation is running.
